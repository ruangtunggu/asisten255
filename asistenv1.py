...
async def main4():
    ...
    async with bos  as bot:
        ...
        async def bacabot(event):
            if event.is_private:
                if event.message.message=="x":bot.disconnect()
...
spt = (os.environ.get("spt")).split()

async def main3(s3,s2,s1,s0):
    pengkoleksi=s0
    global urutpk
    print("koleksi akun :"+s0)  
    pkk=await TelegramClient(StringSession(s3),s2,s1)
...
async def main2():
    print("masuk2")  
    as2=await TelegramClient(StringSession(AccAc),AccAb,AccAa)
...
async def main():
    print("masuk1")  
    as1=await TelegramClient(StringSession(AccAc),AccAb,AccAa)
    async with as1 as asisten1:
        await asisten1.send_message(kgb,"check system from dyno",schedule=timedelta(seconds=20))#(minutes=1))
